...
class environment:
    def __init__(self):
        ...
        self.attribute_frequency = defaultdict(lambda: defaultdict(int))
        # self.attribute_frequency = defaultdict(int)
        self.valid_states = self.generate_actions()
        self.valid_actions = self.valid_states
    ...
    def test(self, filename):
        ...
        # for index, row in df.iterrows():
        #     cur_state = self.get_state(row['Attribute'])
        #     self.attribute_frequency[self.valid_states[tuple(cur_state)]] += 1

        for index in range(len(df)-1):
            # cur_states = ast.literal_eval(df.iloc[index]['Attribute'])
            # next_states = ast.literal_eval(df.iloc[index + 1]['Attribute'])

            cur_states = self.get_state(df.iloc[index]['Attribute'])
            next_states = self.get_state(df.iloc[index+1]['Attribute'])
            if(self.valid_states[tuple(cur_states)] == self.valid_states[tuple(next_states)]):
                continue
                # print("Here")

            self.attribute_frequency[self.valid_states[tuple(cur_states)]][self.valid_states[tuple(next_states)]] += 1
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        for task in tasks:
            ...
            # cnt = 0
            # for items in env.attribute_frequency.values():
            #     if items < 5:
            #         cnt += 1
            # print(len(env.attribute_frequency) - cnt)
            sum = 0
